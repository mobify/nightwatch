...
function Nightwatch(options) {
  ...
  this.loadProtocolActions()
      .loadCommands()
      .loadAssertions(this.assert, true)
      .loadAssertions(this.verify, false);
    ...
    if (assertModule.hasOwnProperty(prop)) {
      parent[prop] = (function(prop) {
          return function() {
            ...
            return self.assertion(passed, actual, expected, message, abortOnFailure);
          };
      })(prop);
    }
  }

  this.loadAssertionFiles(parent, abortOnFailure);

  if (this.options.custom_assertions_path) {
    this.loadCustomAssertions(parent, abortOnFailure);
  }
